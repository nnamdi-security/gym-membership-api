...
#         first = service.run(TEST_RUN_DATE)

#     with Session(engine) as session:
#         service = build_daily_job_service(session)

#         second = service.run(TEST_RUN_DATE)
...
#         assert expired_membership.status == MembershipStatus.EXPIRED

#         assert reminder_membership.status == MembershipStatus.ACTIVE
...
#             job_run_repository=JobRunRepository(session),
#             membership_repository=MembershipRepository(session),
#             reminder_repository=FailingReminderRepository(session),
#         )

#         with pytest.raises(RuntimeError):
#             service.run(TEST_RUN_DATE)
