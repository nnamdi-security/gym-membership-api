...
class WebhookService:
    ...
    def process_payment_event(
        ...
        existing_event = self.processed_event_repository.get_by_event_id(event.event_id)

        if existing_event is not None:
            return WebhookProcessResult(status="duplicate")

        if event.event_type != "payment.succeeded":
            return self._record_ignored_event(event)

        payment = self.payment_repository.get_by_reference_for_update(event.reference)
        ...
        if payment.status == PaymentStatus.SUCCEEDED:
            return self._record_already_confirmed_event(event)

        membership = self.membership_repository.get_by_id(payment.membership_id)
        ...
        membership.end_date = activation_date + timedelta(days=plan.period_days)
        ...
        try:
            ...
            self.processed_event_repository.add(processed_event)
            ...
            duplicate = self.processed_event_repository.get_by_event_id(event.event_id)
        ...
        return WebhookProcessResult(status="processed")
    ...
    def _record_orphan_event(
        ...
        return WebhookProcessResult(status="orphan")
    ...
    def _record_already_confirmed_event(
        ...
        return WebhookProcessResult(status="already_confirmed")
